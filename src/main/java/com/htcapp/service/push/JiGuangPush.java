...
public class JiGuangPush {
    ...
    public  void jiGuangPush(String id,JiGuangPushData data){
        PushResult result = push(id, JsonUtils.objectToString(data));
        if(result != null && result.isResultOK()){
            log.info("针对别名" + id + "的信息推送成功！");
        }else{
            log.info("针对别名" + id + "的信息推送失败！");
        }

    }

    public  void jiGuangPush(Integer id,JiGuangPushData data){

        PushResult result = push(String.valueOf(id), JsonUtils.objectToString(data));
        if(result != null && result.isResultOK()){
            log.info("针对别名" + id + "的信息推送成功！");
        }else{
            log.info("针对别名" + id + "的信息推送失败！");
